...
namespace TagReader.RFIDReader
{
    public class ReaderWrapper
    {
        ...
        public static void AppendRowToDatatable(ref TagStatus reportedTagStatus)
        {
            ...
            //row["TimeStamp(s)"] = reportedTagStatus.TimeStamp;
            //row["TimeStamp"] = currentTime.ToString("u");
        ...
        public static bool CalculateVelocity(out double velocity, ref TagStatus currTagStatus)
        {
            ...
            if (ReportHistory.ContainsKey(key))
            {
                ...
                var phaseChange = currTagStatus.PhaseRadian - ReportHistory[key].PhaseDegree* Convert2Radian;
                ////Console.WriteLine("phaseChanges: ");
                //if (phaseChange > 0.3)
                //{ Console.WriteLine(phaseChange.ToString()); }
        ...
        public static void reader_OnRoAccessReportReceived(MSG_RO_ACCESS_REPORT msg)
        {
            ...
            foreach (PARAM_TagReportData t in msg.TagReportData)
            {
                ...
                if (t.FirstSeenTimestampUTC != null)
                {
                    ...
                    var time = currTagStatus.TimeStamp; //ding
                    MainForm.Invoke(method: new Action(() =>
                    {
...
;
            ...
            cmd.C1G2Filter[0].C1G2TagInventoryMask.TagMask = LLRPBitArray.FromHexString("E20006"); //26100025//19500024//16600023//15500022//21800021
